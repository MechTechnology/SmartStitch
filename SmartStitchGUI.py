...
class SmartStitchGUI(Tk):
    ...
    def setup_paths_frame(self):
        ...
        batch_checkbox = ttk.Checkbutton(paths_frame, variable=self.enable_batch_mode,
                                         text='Batch Mode [Input folder contains multiple chapter folders]',
    ...
    def setup_basic_settings_frame(self):
        ...
        type_dropdown = ttk.Combobox(basic_settings_frame, textvariable=self.output_files_type,
                                     values=('.png', '.jpg', '.webp', '.bmp', '.tiff', '.tga'))
    ...
    def setup_advanced_settings_frame(self):
        ...
        senstivity_field['validatecommand'] = (
        senstivity_field.register(self.validate_percentage_only), '%P', '%d', '%s')
        ...
        ignorable_pixels_field = ttk.Entry(shown_settings_frame, textvariable=self.ignorable_edges_pixels,
                                           validate='all')
        ignorable_pixels_field.bind("<Any-KeyRelease>", self.save_app_settings)
        ignorable_pixels_field['validatecommand'] = (
        ignorable_pixels_field.register(self.validate_nums_only), '%P', '%d', '%s')
        ...
        scan_line_field['validatecommand'] = (
        scan_line_field.register(self.validate_onetotwenty_only), '%P', '%d', '%s')
        ...
        width_enforce_dropdown = ttk.Combobox(shown_settings_frame, textvariable=self.width_enforce_type, values=(
        'No Width Enforcement', 'Automatic Uniform Width', 'User Customized Width'))
        width_enforce_dropdown.bind("<<ComboboxSelected>>",
                                    lambda event: self.update_width_mode(widthfieldtitle, widthfield))
        ...
        show_subprocess_checkbox = ttk.Checkbutton(shown_settings_frame, variable=self.show_subprocess_settings,
                                                   text='Show Subprocess Settings [For Experienced Users Only]',
                                                   command=lambda: self.subprocess_setting_toggle(
                                                       subprocess_setting_frame))
        ...
        show_advanced_checkbox = ttk.Checkbutton(advanced_settings_frame, variable=self.show_advanced_settings,
                                                 text='Show Advanced Settings',
                                                 command=lambda: self.advanced_settings_toggle(shown_settings_frame,
                                                                                               subprocess_setting_frame))
    ...
    def setup_subprocess_frame(self):
        ...
        show_advanced_checkbox = ttk.Checkbutton(subprocess_frame, variable=self.enable_subprocess_execution,
                                                 text='Run the following subprocess after stitching is complete',
                                                 command=lambda: self.save_app_settings)
        show_advanced_checkbox.grid(row=0, column=0, columnspan=2, pady=(2, 5), sticky="new")
        ...
        # Setup of Back Mode Selector/Checkbox.
        argument_hint_label = ttk.Label(subprocess_frame, foreground='blue',
                                        text='Use [stitched] to pass the stitcher output directory, Use [processed] to pass a custom process output directory ',
    ...
    def setup_console_frame(self, Frame):
        ...
        console_horizental_scroll = ttk.Scrollbar(console_main_frame, orient=HORIZONTAL,
                                                  command=self.console_canvas.xview)
        ...
        self.console_canvas.bind('<Configure>',
                                 lambda e: self.console_canvas.configure(scrollregion=self.console_canvas.bbox("all")))
        self.set_mousewheel(self.console_canvas,
                            lambda e: self.console_canvas.yview_scroll(self.mousewheel_handler(e), "units"))
    ...
    def setup_action_frame(self):
        ...
        self.actionbutton = ttk.Button(action_frame, text='Start Process', style="Accent.TButton",
                                       command=self.run_stitch_process_async)
    ...
    def stitch_process(self):
        ...
        folder_paths = ssc.get_folder_paths(self.enable_batch_mode.get(), self.input_folder.get(),
        ...
        for path in folder_paths:
            ...
            self.update_gui_progress("Working - Slicing Combined Image into Finalized Images!",
                                     (10 / self.num_of_inputs))
            final_images = ssc.split_image(combined_image, self.split_height.get(), self.slicing_senstivity.get(),
                                           self.ignorable_edges_pixels.get(), self.scan_line_step.get())
